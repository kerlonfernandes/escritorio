...
class Database {
    ...
    public function execute_non_query($sql, $parameters = null){
        ...
        try {
            ...
            $last_inserted_id = $connection->lastInsertId();  // This is where we get the last inserted ID
            $connection->commit();
        } catch (PDOException $err) {
            $connection->rollBack();
            return $this->_result('error', $err->getMessage(), $sql, null, 0, null);
    ...
    public function getLastInsertId() {
        return $this->_connection->lastInsertId();  // Return the last insert ID
    }
